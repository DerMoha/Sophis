...
class _GoalsSetupScreenState extends State<GoalsSetupScreen> {
  ...
  void _loadExistingData() {
    ...
    if (provider.profile != null) {
      ...
      if (p.weight != null) {
        final displayWeight =
            UnitConverter.displayWeight(p.weight!, unitSystem);
        _weightController.text = displayWeight.toStringAsFixed(1);
      }
      if (p.height != null) {
        final displayHeight =
            UnitConverter.displayHeight(p.height!, unitSystem);
      ...
      if (p.targetWeight != null) {
        final displayTargetWeight =
            UnitConverter.displayWeight(p.targetWeight!, unitSystem);
  ...
  void _calculateFromProfile() {
    ...
    final profile = UserProfile(
      weight: weightInput != null
          ? UnitConverter.inputToKg(weightInput, unitSystem)
          : null,
      height: heightInput != null
          ? UnitConverter.inputToCm(heightInput, unitSystem)
          : null,
      age: _parseInt(_ageController.text),
      gender: _gender,
      activityLevel: _activityLevel,
      goal: _goal,
      targetWeight: targetWeightInput != null
          ? UnitConverter.inputToKg(targetWeightInput, unitSystem)
          : null,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final weightLabel =
        unitSystem == UnitSystem.imperial ? l10n.weightLb : l10n.weightKg;
    final heightLabel =
        unitSystem == UnitSystem.imperial ? l10n.heightIn : l10n.heightCm;
    final targetWeightLabel = unitSystem == UnitSystem.imperial
        ? l10n.targetWeightLb
        : l10n.targetWeight;
